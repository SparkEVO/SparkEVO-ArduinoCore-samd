...
void SPIClassSAMD::dmaAllocate(void) {
...
volatile uint32_t *SPIClassSAMD::getDataRegister(void) {
  int8_t idx = _p_sercom->getSercomIndex();
  return (idx >= 0) ? sercomData[idx].data_reg: NULL;
}

int SPIClassSAMD::getDMAC_ID_TX(void) {
  int8_t idx = _p_sercom->getSercomIndex();
  return (idx >= 0) ? sercomData[idx].dmac_id_tx : -1;
}

int SPIClassSAMD::getDMAC_ID_RX(void) {
  int8_t idx = _p_sercom->getSercomIndex();
